function getHeaders(nameOrHeaders) {
  nameOrHeaders = nameOrHeaders !== null && nameOrHeaders !== void 0 ? nameOrHeaders : "RateLimit";
  return !nameOrHeaders || typeof nameOrHeaders === "string"
    ? [`X-${nameOrHeaders}-Limit`, `X-${nameOrHeaders}-Remaining`, `X-${nameOrHeaders}-Reset`]
    : nameOrHeaders;
}
...
const rateLimited = ({ id, response }) => {
  return new Response(
    ...
    {
      status: 429,
      headers: Object.assign(Object.assign({}, Object.fromEntries(response.headers)), {
...
async function rateLimit(context) {
  ...
  let countOrRes;
  try {
    countOrRes = await count(Object.assign(Object.assign({}, context), { key }));
  } catch (err) {
...
export const initRateLimit = (fn) => async (request, response) => {
